...
12. [Passo 9 — Validar a aplicação](#12-passo-9--validar-a-aplicação)
13. [Passo 10 — Colocar em produção (go-live)](#13-passo-10--colocar-em-produção-go-live)
14. [Passo 11 — Descarte do legado (somente após estabilização)](#14-passo-11--descarte-do-legado-somente-após-estabilização)
15. [Plano de rollback (como voltar atrás)](#15-plano-de-rollback-como-voltar-atrás)
...
O comando executa, em sequência, **5 fases auditáveis e reversíveis até a última**:
...
### Perguntas do assistente (todas por seleção — você não digita texto)

O comando é um **assistente interativo**. As perguntas são **apenas operacionais** e respondidas por **seleção** (Sim/Não ou escolha de opção), nunca por digitação livre — o que elimina erros de digitação:
...
| `--descartar-legado` | Remove os schemas `legacy_*` ao final — **irreversível** (ver Seção 14) |
...
> **Se algo falhar:** o comando **interrompe na primeira falha** e informa a fase. O legado **não é destruído** — ele continua preservado em `legacy_pei`. Corrija a causa indicada e execute novamente. Em último caso, restaure o backup (Seção 15).

---

## 12. Passo 9 — Validar a aplicação
...
## 13. Passo 10 — Colocar em produção (go-live)
...
## 14. Passo 11 — Descarte do legado (somente após estabilização)
...
## 15. Plano de rollback (como voltar atrás)
...
- [ ] Migração real executada com sucesso (Passo 8)
- [ ] Dados validados na aplicação; login testado (Passo 9)
- [ ] Servidor web apontando para `public/` do projeto novo; caches/otimização aplicados (Passo 10)
- [ ] Sistema liberado aos usuários
- [ ] (Dias depois) Legado descartado após estabilização (Passo 11)
...
| Tela em branco / assets sem estilo | `npm run build` não executado | Rode `npm install && npm run build` |
| Permissão negada ao gravar logs/cache | Permissões de `storage/` e `bootstrap/cache/` | Ajuste permissões/propriedade (Passo 10.2) |
...
> **Resumo em uma frase:** faça o backup, preserve o código antigo, clone o novo, configure o `.env` para o mesmo banco, instale as dependências, gere a chave, simule, migre, valide e só então descarte o legado. Seguindo esta ordem, a transição é segura, reversível e **sem redigitação de dados**.
